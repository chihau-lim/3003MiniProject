...
static  void  AppTaskTimerControl(void)
{
    ...
    // Wait for the timeout information in the message queue
      ...
      // msec = time % 1000;
       ...
       OSTimeDlyHMSM(0u, 1u,0u, 0u, OS_OPT_TIME_HMSM_STRICT, &err);
       // Wait until timeout 
...
static  void  AppTaskRobotControl (void  *p_arg)
{
     ...
     motor_dir  = 0;
     motor_speed = 40u; 
     motor_seg = 28u;       // Tell the motors to run forward
    ...
    msg_timer =  60000;      // Generate 1 minute time out
    OSTaskQPost(&AppTaskTimerControlTCB,
     ...
     while(1){
       ...
       case 2: //This is timer expiry
                    ...
                    timer_state = 1; 
                    while(1);
                   // Indicate that timer has expired                    
                    // and distance has not expired
...
static  void  AppTaskLED(void)
{
     ...
     cpu_time = 0;
     LED_Off(0);
     while(cpu_time < 60){ //adjust 30 for 30s,60 for 60s
