...
static  void  AppTaskRobotControl (void  *p_arg)
{
     ...
     motor_dir  = 0;
     motor_speed = 30u; 
     motor_seg = 11u;       // Tell the motors to run forward
    ...
    msg_timer =  30000;     // Generate timeout
...
//         if(rx_val == 0 & state == 0){
//            motor_dir  = 0;          //straight
//           motor_speed = 50u; 
//           motor_seg = 8u;
//         }
         if(rx_val == 0 & state == 0){            // This means it has completed the turn
           motor_dir  = 3;           // Go Left
           motor_speed = 50u; 
           motor_seg =3u;
         }
         ...
         if(rx_val == 0 & state == 10){
            motor_dir  = 3;          //left
           motor_speed = 55u; 
           motor_seg = 2u;
         }
         
           if(rx_val == 3 & state == 11){
            motor_dir  = 0;          //straight
           motor_speed = 20u; 
           motor_seg = 20u;
         }
          
     /*    if(rx_val == 0 & state == 12){
            motor_dir  = 3;          //left
           motor_speed = 50u; 
           motor_seg = 3u;
         }
         
          if(rx_val == 3 & state == 13){
            motor_dir  = 0;          //straight
           motor_speed = 23u; 
           motor_seg = 8u;
         }
    */     
       ...
       case 2: //This is timer expiry
               ...
               while(1);                         // and distance has not expired
...
static  void  AppTaskLED(void)
{
     ...
     cpu_time = 0;
     LED_Off(0);
     while(cpu_time < 30){ //adjust 30 for 30s,60 for 60s
       OSTimeDlyHMSM(0u, 0u, 1u, 0u, OS_OPT_TIME_HMSM_STRICT, &err);
       cpu_time = cpu_time + 1;
       if(cpu_time % 2 == 1)
           LED_On(0);
